...
// ─── Types ────────────────────────────────────────────────────────────────────
type ImageType = 'brand' | 'accord' | 'note' | 'product';
...
type ProgressMap = Record<string, string>; // sourceUrl -> cloudinary secure_url
...
function buildCatalog(perfumes: any[]): Map<string, UrlEntry> {
  const catalog = new Map<string, UrlEntry>();

  function add(url: string, entry: Omit<UrlEntry, 'sourceUrl'>) {
...
// ─── Upload ───────────────────────────────────────────────────────────────────
async function uploadOne(entry: UrlEntry): Promise<string> {
...
function makeTypeStats(): TypeStats {
...
async function runBatch(
  ...
  byType:   Record<ImageType, TypeStats>
) {
  let idx   = 0;
  let done  = 0;
...
// ─── Rewrite JSON ─────────────────────────────────────────────────────────────
function rewriteJson(perfumes: any[], progress: ProgressMap): any[] {
  function remap(url: string): string {
  ...
  return perfumes.map((p) => {
    ...
    if (Array.isArray(updated.mainAccords)) {
      updated.mainAccords = updated.mainAccords.map((a: any) => ({
        ...a,
        image: remap(a.image),
      }));
    }
    ...
    if (updated.notes) {
      ...
      for (const tier of ['topNotes', 'middleNotes', 'baseNotes']) {
        if (Array.isArray(notes[tier])) {
          notes[tier] = notes[tier].map((n: any) => ({
            ...n,
            image: remap(n.image),
          }));
        }
...
async function main() {
  ...
  const perfumes: any[] = JSON.parse(fs.readFileSync(DATA_FILE, 'utf-8'));
  ...
  // Count by type
  const countByType: Record<ImageType, number> = { brand: 0, accord: 0, note: 0, product: 0 };
  ...
  // Upload
  const byType: Record<ImageType, TypeStats> = {
    brand:   makeTypeStats(),
  ...
  log('  ├─────────────┼────────┼──────────┼─────────┼────────┤');
  for (const [type, s] of Object.entries(byType) as [ImageType, TypeStats][]) {
